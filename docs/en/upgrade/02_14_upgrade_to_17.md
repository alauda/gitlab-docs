...
Data migration Path
According to the [official upgrade path](https://gitlab-com.gitlab.io/support/toolbox/upgrade-path/?current=14.0.12&target=17.8.5&distro=docker&edition=ce) documentation, the data migration path is as follows:
...
3. Upgrade the all-in-one deployed GitLab to 17.8.5;
4. Backup the all-in-one deployed 17.8.5;
5. Restore the all-in-one backup data to the platform-deployed 17.8.5.
...
#### Upgrade all-in-one deployed gitlab to 17.8.5
...
7. Upgrade 15.11.13 to 16.3.9: Replace the image tag with 16.3.9-ce.0, other operations are the same as "14.0.12 to 14.3.6".
8. Upgrade 16.3.9 to 16.7.10: Replace the image tag with 16.7.10-ce.0, other operations are the same as "14.0.12 to 14.3.6".
9. Upgrade 16.7.10 to 16.11.10: Replace the image tag with 16.11.10-ce.0, other operations are the same as "14.0.12 to 14.3.6".
...
12. Upgrade 17.5.5 to 17.8.5: Replace the image tag with 17.8.5-ce.0, other operations are the same as "14.0.12 to 14.3.6".

### Backup in all-in-one 17.8.5

1. In the 17.8.5 deployment pod, execute the backup.
...
# Backup gitlab 17.8.5 deployment's gitlab-secrets
...
### Restore all-in-one backup data to platform deployed 17.8.5

1. Deploy gitlab 17.8.5 on the platform using the operator and enable the toolbox (note that it needs to be in the same namespace as the old instance), then restore the data to the gitlab 17.8.5 deployed by the operator (set the name of the gitlab deployed on the platform to the environment variable NEW_GITLAB_NAME).
...
# Close the external access of the 17.8.5 gitlab
...
# Note that this step will delete the backup file, you can back it up in advance: cp /srv/gitlab/tmp/backups/17302758xx_xxxx_xx_xx_14.0.12_gitlab_backup.tar /srv/gitlab/tmp/backups/17302758xx_xxxx_xx_xx_14.0.12_gitlab_backup2.tar
...
2. When restoring the backup to the 17.8.5 deployed by the platform, the toolbox container is killed, and the request and limit resources need to be added to the toolbox container (at least 2c4g).

3. The upgrade backup does not include external attachment images and avatars, so the page cannot display attachment images and uploaded avatars after the upgrade. You can replace the 17.8.5 PVC with the 14.0.12 image storage PVC.
