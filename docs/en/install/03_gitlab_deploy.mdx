...
1. **Init Container Requires Root Privileges**: GitLab uses init containers to initialize PVC directory permissions, which requires root privileges that are not allowed under the Restricted policy.

2. **Gitaly Component Requires Special System Calls**: The Gitaly component requires specific system calls that are incompatible with `seccompProfile` configured as `RuntimeDefault`.
...
|---| ---| ---|
| Gitaly Storage | block storage (e.g., Ceph RBD)<br />file storage (e.g., Ceph FS) | For production environments, it is recommended to use block storage with storage performance meeting the following requirements: read IOPS ≥ 8,000 and write IOPS ≥ 2,000. |
| Upload Storage | object storage (e.g., MinIO)<br />file storage (e.g., Ceph FS) | For production environments, it is recommended to use object storage.<br />The sequential read/write throughput of the storage disk (with a 1 MB block size) should not be lower than 100 MiB/s. |
