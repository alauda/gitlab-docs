...
[How to determine if object storage is enabled?](./03_official_backup_and_restore.mdx#applicability)
...
praefect_db_secret_name=$(kubectl get gitlabofficial -n ${GITLAB_NAMESPACE} ${GITLAB_NAME} -o jsonpath='{.spec.helmValues.global.praefect.dbSecret.secret}')
if [ -n "${praefect_db_secret_name}" ]; then
    echo "Found praefect database secret: ${praefect_db_secret_name}"
    secrets_to_label+=(${praefect_db_secret_name})
fi
...
:::warning
If the Redis Secret is managed outside the GitLab instance, for example by `Alauda Cache Service for Redis OSS`, the external controller may remove the backup label added by this script. In that case, Velero will not back up the Redis Secret. Treat externally managed Redis Secrets as external dependencies: back them up manually and restore them into the target namespace before starting the restored GitLab instance.
:::
...
Before restoring, ensure that the target cluster has the `GitlabOfficial` CRD
from a GitLab CE Operator version compatible with the source instance. If the
Operator has never been installed on the target cluster, install a compatible
version first to register the CRD, and then uninstall the Operator as described
below. Keep the `GitlabOfficial` CRD installed during the restore because the
GitLab CR is included in the Velero backup. Redeploy the Operator only after the
restore is complete and the restored GitLab CR has been reviewed and updated.

:::

Restore preparation involves five steps:

1. Restore PostgreSQL Database (Optional)
2. Prepare Redis Secret (Optional)
3. Select a backup to restore
4. Determine target namespace for restoration
5. Uninstall Gitlab CE Operator
...
#### Prepare Redis Secret (Optional)

If the new instance will still use the same Redis instance and Secret name, ensure that the Redis Secret exists in the target namespace. When the Redis Secret is managed outside the GitLab instance, for example by `Alauda Cache Service for Redis OSS`, the external controller may remove the backup label added in the `Label Resources for Backup` step. In that case, Velero will not back up the Redis Secret, and you need to back it up manually and restore or copy it to the target namespace.

If the new instance will use a different Redis instance or Secret name than the old instance, create the Redis Secret for the new instance in the target namespace first. You will update the GitLab CR to use the new Redis connection information in the [Modify GitLab CR Resource](#modify-gitlab-cr-resource) step.
...
#### Modify GitLab CR Resource \{#modify-gitlab-cr-resource}
...
1. **Domain Name**:
   ...
   - **How to modify**: Please refer to [Configure Instance Network Access](../install/03_gitlab_deploy.mdx#network_access) to modify the instance access domain name.
2. **NodePort**:
   ...
   - **How to modify**: Please refer to [Configure Instance Network Access](../install/03_gitlab_deploy.mdx#network_access) to modify the instance access configuration.
3. **Storage Class**:
   ...
   - **How to modify**: Please refer to [Configure Instance Storage](../install/03_gitlab_deploy.mdx#storage) to modify the instance storage configuration.
4. **PostgreSQL Connection Information**:
   ...
   - **How to modify**: Please refer to [Configure PostgreSQL Access Credential Configuration](../install/03_gitlab_deploy.mdx#configure-postgresql-credentials) to modify the PostgreSQL access credential configuration.
5. **Redis Connection Information**:
   - **Applicable scenario**: New instance and old instance plan to use different Redis instances or Secret names
   - **Why modify**: If not modified, the new instance will still connect to the old Redis instance or use the old Redis Secret name
   - **Modification recommendation**: Change the Redis connection information to the correct values
   - **How to modify**: Please refer to [Configure Redis Access Credential Configuration](../install/03_gitlab_deploy.mdx#configure-redis-credentials) to modify the Redis access credential configuration.
